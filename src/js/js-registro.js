...
function validarRegistro(){
    ...
    if(!validarCadena(inputNombre, "Nombre"))
        registroValido = false;
    if (!validarCadena(inputApellido, "Apellido"))
        registroValido = false;
...
function validarCadena(input, campoError){
    if (validator.isEmpty(input.value.trim())){
        mostrarMensajeError(input, `error${campoError}`,`El ${campoError} no puede estar vacio`);
        return false;
    }else{
        mostrarExito(input);
        return true;
...
function validarTelefono(input){
    if (validator.isEmpty(input.value.trim())){
        mostrarMensajeError(input, "errorTelefono", "Ingrese un numero de telefono.");
        return false;
    }
    const valor = input.value.replace(/[+-]/g, "");
    if(valor.length != 10){
        mostrarMensajeError(input, "errorTelefono", "Ingrese un numero de telefono valido.");
        return false;
    }
    mostrarExito(input);
    return true;
...
function validarCorreo(input){
    if (validator.isEmpty(input.value.trim())){
        mostrarMensajeError(input, "errorCorreo", "El correo no puede estar vacio.");
        return false;
    }
    ...
    if (yaRegistrado(input.value)){
        mostrarMensajeError(input, "errorCorreo", "El correo ya se encuentra registrado.")
        return false;
    }
    mostrarExito(input);
    return true;
...
function validarPasswords(input1, input2){
    if(validator.isEmpty(input1.value.trim()) && validator.isEmpty(input2.value.trim())){
    ...
    if(!validator.equals(input1.value, input2.value)){
        mostrarMensajeError(input1,"errorPassword", "Las contraseñas no coinciden");
...
function quitarErrorSucces(){
    document.getElementById("password1").classList.remove("text-danger","text-success");
    document.getElementById("password2").classList.remove("text-danger","text-success");
    document.getElementById("password3").classList.remove("text-danger","text-success");
    document.getElementById("password4").classList.remove("text-danger","text-success");
